...
func (s *GenericAPIServer) InstallMiddlewares() {
	...
	for _, m := range s.middlewares {
		...
		log.Infof("install midleware: %s", m)
		s.Use(mw)
	}
}
...
func (s *GenericAPIServer) Close() {
	// The context is used to inform the server, it has 10 second to finish
...
// ping pings the server to make sure the router is working.
